 ...
 *
 * Description: Magic numbers have spaces between them.
 */
 ...
 * @e_ident: A pointer to an array that has the ELF version.
 */
 ...
 * @e_ident: A pointer to an array that has the ELF ABI version.
 */
 ...
 * @e_ident: A pointer to an array thta has the ELF class.
 */
...
/**
 * main - Displays the contents of an
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	a = read(filde, header, sizeof(Elf64_Ehdr));
	if (a == -1)
	{
		free(header);
		close_elf(filde);
